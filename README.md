...
`User::summary()` makes a wider cut than that: it lists only accounts somebody has signed in to at least once (`scopeLoggedIn`), whoever created them, since an account opened on a customer's behalf is a placeholder for a person until that person uses it. The list's `never-logged-in` checkbox brings the rest back.
