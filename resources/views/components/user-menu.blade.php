@auth
    <div class="dropdown">
        <button class="btn dropdown-toggle text-white" type="button" id="userMenu"
            data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false"
            title="@lang('ig-user::user.detail')">
            <i class="fas fa-fw {{ auth()->user()->role->icon() }}"></i>
        </button>
        <ul class="dropdown-menu" aria-labelledby="userMenu">
            <li><span class="dropdown-item-text">{{ auth()->user()->name }} {!! formatUserRoleLabelSlim(auth()->user()->role) !!}</span></li>
